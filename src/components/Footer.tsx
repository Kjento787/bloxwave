...
export const Footer = () => {
  ...
  return (
    <footer className="bg-background border-t border-border/30 mt-16">
      <div className="w-full px-4 md:px-8 lg:px-12 py-12">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 lg:gap-12 mb-10">
          ...
          {/* Browse */}
          <div>
            <h4 className="font-semibold mb-4 text-sm uppercase tracking-wider text-muted-foreground">Browse</h4>
            <ul className="space-y-2">
              {links.browse.map(({ to, label }) => (
                <li key={`${to}-${label}`}>
                  <Link to={to} className="text-sm text-foreground/70 hover:text-foreground transition-colors">
                    {label}
                  </Link>
                </li>
              ))}
        ...
        <div className="border-t border-border/30 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <p className="text-xs text-muted-foreground">
            Data provided by <a href="https://www.themoviedb.org/" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">TMDB</a>
          </p>
        </div>
      </div>
    </footer>
