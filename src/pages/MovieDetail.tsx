...
const MovieDetail = () => {
  ...
  // Auto-start preview after delay
  useEffect(() => {
      ...
      <section className="relative min-h-screen">
        ...
        <div className="relative container mx-auto px-4 md:px-8 lg:px-12 pt-32 pb-16 min-h-screen flex flex-col justify-end">
          ...
          <div className="max-w-3xl space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {movie.genres.map((genre) => (
                <Link key={genre.id} to={`/genre/${genre.id}`}>
                  <Badge 
                    variant="secondary" 
                    className="hover:bg-primary hover:text-primary-foreground transition-colors px-4 py-1.5 text-sm"
                  >
                    {genre.name}
                  </Badge>
                </Link>
            ...
            {/* Credits */}
            <div className="flex flex-wrap gap-x-8 gap-y-2 pt-4 text-sm">
              {director && (
                <div>
              ...
              {writers.length > 0 && (
                <div>
                  ...
                  <span className="font-semibold">{writers.map(w => w.name).join(", ")}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="container mx-auto px-4 md:px-8 lg:px-12">
          <Tabs defaultValue="cast" className="space-y-8">
            <TabsList className="glass inline-flex h-12 p-1 rounded-xl">
              <TabsTrigger value="cast" className="px-6 rounded-lg">Cast & Crew</TabsTrigger>
              <TabsTrigger value="details" className="px-6 rounded-lg">Details</TabsTrigger>
