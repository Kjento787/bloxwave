...
import {
  ...
  Film, Tv, Star, Edit3, Grid3X3, LayoutList,
} from "lucide-react";
    ...
    <PageTransition>
      <div className="min-h-screen bg-background">
        ...
        <section className="relative pt-20 pb-32 overflow-hidden">
          ...
          >
            <div className="flex flex-col md:flex-row items-center gap-8 py-12">
              <motion.div custom={0} variants={fadeUp} className="relative group">
                <div className="absolute -inset-1 rounded-full bg-gradient-to-br from-primary/40 to-primary/10 blur-md opacity-60" />
                <Avatar className="relative w-32 h-32 md:w-40 md:h-40 border-2 border-primary/30">
                  <AvatarImage src={profile?.avatar_url || undefined} alt="Profile" />
                  <AvatarFallback className="bg-card text-4xl font-bold font-display text-primary">
                    {profile?.display_name?.[0]?.toUpperCase() || <User className="h-16 w-16" />}
                  </AvatarFallback>
                </Avatar>
                <Link
                  to="#settings"
                  className="absolute bottom-2 right-2 p-2 rounded-full bg-primary text-primary-foreground opacity-0 group-hover:opacity-100 transition-opacity shadow-glow"
                >
              ...
              <div className="flex-1 text-center md:text-left space-y-4">
                <motion.div custom={1} variants={fadeUp}>
                  ...
                  {profile?.bio && (
                    <p className="text-lg text-muted-foreground mt-2 max-w-xl">{profile.bio}</p>
                  )}
                </motion.div>
                ...
                <motion.div custom={3} variants={fadeUp} className="flex flex-wrap justify-center md:justify-start gap-3 pt-2">
                  <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-card/80 backdrop-blur-sm border border-border/40">
        ...
        <main className="container mx-auto px-4 md:px-8 lg:px-12 -mt-20 relative z-10 pb-12">
          <div className="mb-8"><UserStatistics /></div>
          <div className="mb-8"><Recommendations /></div>

          <Tabs defaultValue="continue" className="space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <TabsList className="bg-card/80 backdrop-blur-sm border border-border/40 h-12 p-1 rounded-xl">
                <TabsTrigger value="continue" className="gap-2 px-4 rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                  <Clock className="h-4 w-4" />
                  <span className="hidden sm:inline">Continue</span>
                  <Badge variant="secondary" className="ml-1">{continueWatching.length}</Badge>
                </TabsTrigger>
                <TabsTrigger value="watchlist" className="gap-2 px-4 rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                ...
                <TabsTrigger value="settings" className="gap-2 px-4 rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
              ...
              <div className="flex items-center gap-2">
                <Button variant="ghost" size="icon" onClick={() => setViewMode("grid")}
                  className={cn("h-9 w-9 rounded-lg", viewMode === "grid" && "bg-secondary")}>
                  <Grid3X3 className="h-4 w-4" />
                </Button>
                <Button variant="ghost" size="icon" onClick={() => setViewMode("list")}
                  className={cn("h-9 w-9 rounded-lg", viewMode === "list" && "bg-secondary")}>
            ...
            <TabsContent value="continue" className="space-y-4">
              {continueWatching.length === 0 ? (
                <div className="text-center py-20 px-4 rounded-2xl bg-card/60 backdrop-blur-sm border border-border/30">
                  <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-primary/10 flex items-center justify-center shadow-glow">
                    <Clock className="h-10 w-10 text-primary" />
                  </div>
                  <h3 className="text-xl font-bold mb-2 font-display">Nothing in progress</h3>
                  <p className="text-muted-foreground mb-6 max-w-md mx-auto">
                    Start watching movies and TV shows, and they'll appear here.
                  </p>
                  <Link to="/movies">
                    <Button size="lg" className="rounded-xl px-8 shadow-glow">Browse Content</Button>
                  </Link>
                </div>
              ) : (
                <div className="grid gap-3">
                  {continueWatching.map((item, i) => (
                    <motion.div
                      ...
                      className="flex gap-4 p-4 rounded-xl bg-card/60 backdrop-blur-sm border border-border/30 hover:border-primary/30 transition-all group"
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <Link to={`/movie/${item.movieId}`}>
                          <Button className="rounded-lg gap-2 shadow-glow">
            ...
            <TabsContent value="watchlist" className="space-y-4">
              {isWatchlistLoading ? (
                ...
                <div className="text-center py-20 px-4 rounded-2xl bg-card/60 backdrop-blur-sm border border-border/30">
                  ...
                  <h3 className="text-xl font-bold mb-2 font-display">Your list is empty</h3>
                  <p className="text-muted-foreground mb-6 max-w-md mx-auto">
                ...
                >
                  {watchlistWithDetails.map((item, i) => (
                    <motion.div key={item.id} custom={i} variants={fadeUp} className="group relative">
                      <Link
                        to={item.content_type === "tv" ? `/tv/${item.content_id}` : `/movie/${item.content_id}`}
                        className="block rounded-xl overflow-hidden bg-card/60 border border-border/30 transition-all hover:scale-105 hover:border-primary/30 hover:shadow-hover"
                      >
                        <div className="aspect-[2/3] relative">
                          ...
                          <div className="absolute top-2 left-2">
                            <Badge className={cn("text-xs", item.content_type === "tv" ? "bg-secondary/80" : "bg-primary/80")}>
                ...
                <div className="space-y-3">
                  {watchlistWithDetails.map((item, i) => (
                    <motion.div
                      ...
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: i * 0.05 }}
                      className="flex items-center gap-4 p-4 rounded-xl bg-card/60 backdrop-blur-sm border border-border/30 hover:border-primary/30 transition-all group"
                    >
            ...
            <TabsContent value="settings" id="settings">
              <div className="max-w-2xl"><ProfileSettings /></div>
            </TabsContent>
          </Tabs>
        </main>
